...
## Detalle del código de rechazo[​](#detalle-del-código-de-rechazo "Enlace directo al Detalle del código de rechazo")

Cuando se rechaza una transacción se le asocia un mensaje de error que depende del servicio de pago empleado para llevar a cabo la operación. Este mensaje de error a menudo contiene tanto el código de error como su descripción, y puede variar en el tiempo. Para interpretar el error es recomendable basarse en el objeto `error_details` que contiene la transacción en la notificación del pago.
...
Todos los campos son nullables aunque si todos ellos son null, el objeto `error_details` no se incluye en la respuesta (su valor será `null`).

```
"error_details": {    "error_code": "190",    "error_description": "Denegación del emisor sin especificar motivo",    "error_code_scheme": "05",    "error_description_scheme": "Do not honor",    "error_unified": "UNSPECIFIED_REFUSAL"}
```
...
## Listado de errores unificado[​](#listado-de-errores-unificado "Enlace directo al Listado de errores unificado")
...
| PIN REQUIRED                         | PIN\_REQUIRED - Se requiere que el usuario introduzca el PIN                                      | 70,P6 (Visa) 71 (Mastercard)                      |
...
| CARD FROZEN                          | El usuario ha congelado su tarjeta y no puede operar hasta que la reactive                        |                                                   |
| CARD CANCELLED                       | El usuario ha cancelado su cuenta y no volverá a operar con ésta de nuevo                         |                                                   |

## Merchant advice code (MAC)[​](#merchant-advice-code-mac "Enlace directo al Merchant advice code (MAC)")

Solo en el caso de tarjetas Mastercard, es posible que el campo `error_code_scheme` contenga dos códigos de error separados por una coma. El segundo valor es el campo MAC (`Merchant Advice Code`) a través del cual la marca nos informa del momento a partir del cual deberíamos intentar volver a cobrar al usuario.
